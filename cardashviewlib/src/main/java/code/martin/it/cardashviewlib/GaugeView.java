...
/*******************************************************************************
 * This file was included in different package than original together with
 * other widget in a single library
 * Paolo Martinello 2015
 *******************************************************************************/
...
public class GaugeView  extends View {
    ...
    private void readAttrs(final Context context, final AttributeSet attrs, final int defStyle) {
        ...
        mShowOuterBorder = a.getBoolean(R.styleable.CardashViewLib_gaugeShowOuterBorder, SHOW_OUTER_BORDER);
        mShowOuterRim = a.getBoolean(R.styleable.CardashViewLib_gaugeShowOuterRim, SHOW_OUTER_RIM);
        mShowInnerRim = a.getBoolean(R.styleable.CardashViewLib_gaugeShowInnerRim, SHOW_INNER_RIM);
        mShowNeedle = a.getBoolean(R.styleable.CardashViewLib_gaugeShowNeedle, SHOW_NEEDLE);
        mShowScale = a.getBoolean(R.styleable.CardashViewLib_gaugeShowScale, SHOW_SCALE);
        mShowRanges = a.getBoolean(R.styleable.CardashViewLib_gaugeShowRanges, SHOW_RANGES);
        mShowText = a.getBoolean(R.styleable.CardashViewLib_gaugeShowTheText, SHOW_TEXT);

        mOuterShadowWidth = mShowOuterShadow ? a.getFloat(R.styleable.CardashViewLib_gaugeOuterShadowWidth, OUTER_SHADOW_WIDTH) : 0.0f;
        mOuterBorderWidth = mShowOuterBorder ? a.getFloat(R.styleable.CardashViewLib_gaugeOuterBorderWidth, OUTER_BORDER_WIDTH) : 0.0f;
        mOuterRimWidth = mShowOuterRim ? a.getFloat(R.styleable.CardashViewLib_gaugeOuterRimWidth, OUTER_RIM_WIDTH) : 0.0f;
        mInnerRimWidth = mShowInnerRim ? a.getFloat(R.styleable.CardashViewLib_gaugeInnerRimWidth, INNER_RIM_WIDTH) : 0.0f;
        mInnerRimBorderWidth = mShowInnerRim ? a.getFloat(R.styleable.CardashViewLib_gaugeInnerRimBorderWidth, INNER_RIM_BORDER_WIDTH) : 0.0f;

        mNeedleWidth = a.getFloat(R.styleable.CardashViewLib_gaugeNeedleWidth, NEEDLE_WIDTH);
        mNeedleHeight = a.getFloat(R.styleable.CardashViewLib_gaugeNeedleHeight, NEEDLE_HEIGHT);

        mScalePosition = (mShowScale || mShowRanges) ? a.getFloat(R.styleable.CardashViewLib_gaugeScalePosition, SCALE_POSITION) : 0.0f;
        mScaleStartValue = a.getFloat(R.styleable.CardashViewLib_gaugeScaleStartValue, SCALE_START_VALUE);
        mScaleEndValue = a.getFloat(R.styleable.CardashViewLib_gaugeScaleEndValue, SCALE_END_VALUE);
        mScaleStartAngle = a.getFloat(R.styleable.CardashViewLib_gaugeScaleStartAngle, SCALE_START_ANGLE);

        mDivisions = a.getInteger(R.styleable.CardashViewLib_gaugeDivisions, SCALE_DIVISIONS);
        mSubdivisions = a.getInteger(R.styleable.CardashViewLib_gaugeSubdivisions, SCALE_SUBDIVISIONS);

        if (mShowRanges) {
            mTextShadowColor = a.getColor(R.styleable.CardashViewLib_gaugeTextShadowColor, TEXT_SHADOW_COLOR);
            final int rangesId = a.getResourceId(R.styleable.CardashViewLib_gaugeRangeValues, 0);
            final int colorsId = a.getResourceId(R.styleable.CardashViewLib_gaugeRangeColors, 0);
            readRanges(context.getResources(), rangesId, colorsId);
        }

        if (mShowText) {
            final int textValueId = a.getResourceId(R.styleable.CardashViewLib_gaugeTextValue, 0);
            final String textValue = a.getString(R.styleable.CardashViewLib_gaugeTextValue);
            mTextValue = (0 < textValueId) ? context.getString(textValueId) : (null != textValue) ? textValue : "";

            final int textUnitId = a.getResourceId(R.styleable.CardashViewLib_gaugeTextUnit, 0);
            final String textUnit = a.getString(R.styleable.CardashViewLib_gaugeTextUnit);
            mTextUnit = (0 < textUnitId) ? context.getString(textUnitId) : (null != textUnit) ? textUnit : "";
            mTextValueColor = a.getColor(R.styleable.CardashViewLib_gaugeTextValueColor, TEXT_VALUE_COLOR);
            mTextUnitColor = a.getColor(R.styleable.CardashViewLib_gaugeTextUnitColor, TEXT_UNIT_COLOR);
            mTextShadowColor = a.getColor(R.styleable.CardashViewLib_gaugeTextShadowColor, TEXT_SHADOW_COLOR);

            mTextValueSize = a.getFloat(R.styleable.CardashViewLib_gaugeTextValueSize, TEXT_VALUE_SIZE);
            mTextUnitSize = a.getFloat(R.styleable.CardashViewLib_gaugeTextUnitSize, TEXT_UNIT_SIZE);
        }
    ...
    public Paint getDefaultFacePaint() {
        ...
        //		new int[]{Color.rgb(50, 132, 206),
        //	Color.rgb(36, 89, 162),
        //	Color.rgb(27, 59, 131)}, new float[]{0.5f, 0.96f, 0.99f}, TileMode.MIRROR));
        //paint.setStyle(Style.FILL);
        int c1=Color.rgb(182, 106, 18);
        int c2=Color.rgb(57, 36, 0);
        int c3=mShowNeedle?c1:c2;
        paint.setShader(new RadialGradient(0.5f, 0.5f, mFaceRect.width() / 2,
                new int[]{c3,
                        Color.rgb(57, 36, 0),
                        Color.rgb(57, 36, 0),
                        Color.rgb(124, 66, 0)}, new float[]{0.05f,0.2f, 0.94f, 0.99f}, Shader.TileMode.MIRROR));
        return paint;
    }
    ...
    public Paint getDefaultFaceShadowPaint() {
        ...
        //paint.setStyle(Style.FILL);
        //paint.setShader(new RadialGradient(0.5f, 0.5f, mFaceRect.width() / 2.0f, new int[]{0x392400,
        //	0x392400, 0x392400, 0x392400},
    ...
    private void drawScale(final Canvas canvas) {
        ...
        float textAngle=mScaleRotation;
